...
axiosInstance.interceptors.request.use(
    (config) => {
        ...
        console.log('Request: - api.js:28', config.method.toUpperCase(), config.url);

        return config;
    },
    (error) => {
        console.error('Request Error: - api.js:33', error);
...
axiosInstance.interceptors.response.use(
    (response) => {
        ...
        console.log('Response: - api.js:42', response.config.url, response.status);

        // Return data trực tiếp để dễ sử dụng
        return response.data;
    },
    (error) => {
        console.error('Response Error: - api.js:48', error);
